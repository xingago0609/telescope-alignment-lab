...
function choose(i){if(!Number.isInteger(i)||!targets[i])throw Error('無效的標的');state.target=i;state.selected=0;state.lock=0;state.turns=[0,0,0];state.moves=0;state.completed=false;message='主鏡已置中並固定。請先將固定螺絲向外轉 3 格。';render()}
function lock(dir){if(state.target===null||state.completed)return;state.lock=Math.max(0,Math.min(3,state.lock+dir));message=state.lock===3?'固定已解除。試著轉動 A、B 或 C，觀察標的在尋星鏡中的移動。':state.lock===0?'已鎖緊。按「檢查對準」確認結果。':'繼續轉動固定螺絲；完全鬆開後才能校準。';render()}
function turn(i,dir){if(!Number.isInteger(i)||i<0||i>2||![-1,1].includes(dir))throw Error('無效的螺絲操作');if(state.lock!==3||state.completed||state.target===null)throw Error('請先解除固定');const step=$('fine').checked?.25:1;const n=state.turns[i]+dir*step;if(Math.abs(n)>12){message='這顆螺絲已達模擬行程上限，試著調整另一顆。';render();return}const before=offset().d;state.selected=i;state.turns[i]=n;state.moves++;const after=offset().d;message=after<=3?'標的已進入中心容許範圍。現在向內轉固定螺絲 3 格，再檢查對準。':after<before?'標的更靠近中心了。繼續觀察，接近時可勾選「精細調整」。':'標的離中心更遠了。可反向旋轉，或改調另一顆螺絲。';render()}
function check(){if(state.target===null)return;if(state.lock!==0){message='檢查前，請將固定螺絲向內轉至鎖緊。'}else if(offset().d>3){message='仍有偏移。重新向外轉 3 格解除固定，再調整標的位置。'}else{state.completed=true;message='校準通過！固定螺絲已鎖緊，兩個視野的標的都在中心。'}render()}
function reset(){state.target=null;state.completed=false;message='';render();window.scrollTo({top:0,behavior:'smooth'})}
function targetLayout(){if(!img.naturalWidth)return;const w=$('landscape').clientWidth,h=$('landscape').clientHeight,s=Math.max(w/img.naturalWidth,h/img.naturalHeight),ox=(w-img.naturalWidth*s)/2,oy=(h-img.naturalHeight*s)/2;$('targets').innerHTML=targets.map((t,i)=>`<button class="target" data-target="${i}" style="left:${(t.x*img.naturalWidth*s+ox)/w*100}%;top:${(t.y*img.naturalHeight*s+oy)/h*100}%" aria-label="選擇${t.name}"><i>+</i><span>${t.name}</span></button>`).join('')}
function draw(id,isFinder){
 ...
 if(img.complete&&img.naturalWidth&&state.target!==null){
  const t=targets[state.target],o=offset(),scale=isFinder?2.2:3.6;
...
function render(){const chosen=state.target!==null,o=offset(),stage=!chosen?0:state.completed?4:state.lock<3&&state.moves===0?1:o.d<=3?3:2;$('steps').innerHTML=['選擇標的','解除固定','三點校準','鎖緊確認'].map((s,i)=>`<div class="step ${i===stage?'active':i<stage?'done':''}" ${i===stage?'aria-current="step"':''}><b>${i<stage?'✓':'0'+(i+1)}</b>${s}</div>`).join('');$('selection').hidden=chosen;$('exercise').hidden=!chosen;$('success').hidden=!state.completed;$('chapter').textContent=!chosen?'01 / OBSERVE':state.completed?'COMPLETE / VERIFIED':stage===1?'02 / UNLOCK':stage===3?'04 / VERIFY':'03 / ALIGN';$('title').textContent=!chosen?'先找一個清楚的遠方標的。':state.completed?'很好，尋星鏡已經對準。':'讓兩個視野指向同一個位置。';$('intro').textContent=!chosen?'選擇景色中的固定物體，以尖端或邊角作為對準位置。':`目前標的：${targets[state.target].name} · 主鏡保持固定，只調整尋星鏡。`;$('restart').hidden=!chosen;if(!chosen){targetLayout();return}$('lockState').textContent=['已鎖緊','已鬆開 1 格','已鬆開 2 格','已解除固定'][state.lock];$('lockTicks').innerHTML=[0,1,2].map(i=>`<i class="${i<state.lock?'on':''}"></i>`).join('');$('loosen').disabled=state.lock===3||state.completed;$('tighten').disabled=state.lock===0||state.completed;document.querySelectorAll('[data-screw]').forEach(b=>b.disabled=state.lock!==3||state.completed);state.turns.forEach((t,i)=>$('out'+i).textContent=(t>0?'+':'')+t);$('fine').disabled=state.completed;$('check').disabled=state.completed;$('finderStatus').textContent=o.d<=3?'已進入中心範圍':'尚未對準';$('feedbackTitle').textContent=state.completed?'校準通過':state.lock!==3&&state.moves===0?'先鬆開固定螺絲':o.d<=3?'對準了，鎖緊後再確認':'觀察偏移，微調螺絲';$('meter').style.width=Math.max(3,100-o.d/100*100)+'%';$('distanceLabel').textContent=o.d<=3?'中心範圍 ✓':'偏移程度 '+(o.d>40?'較大':o.d>15?'中等':'接近中心');$('hint').textContent=message;renderMechanism();draw('mainView',false);draw('finderView',true)}
